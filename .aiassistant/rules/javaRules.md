...
| 규칙 ID | 수준 | 적용 절 | 주요 검증 |
| --- | --- | --- | --- |
| `JAVA-COMMENT-001` | MUST | 1, 7, 8 | 주석 문체, Javadoc, 로직 주석 검토 |
| `JAVA-CONFIG-001` | MUST | 2 | 하드코딩과 설정 바인딩 검사 |
| `JAVA-VALID-001` | MUST | 3 | 입력, Null 및 예외 검증 |
| `JAVA-TX-001` | MUST | 4 | 읽기·쓰기 트랜잭션 경계 검토 |
| `JAVA-PARAM-001` | MUST | 5 | 파라미터 가중치와 정렬 검토 |
| `JAVA-API-001` | MUST | 6, 9 | URI, 공통 응답 및 OpenAPI 계약 검토 |
| `JAVA-NAME-001` | MUST | 10 | 메서드 접두사와 25자 제한 검사 |
| `JAVA-QUALITY-001` | MUST | 11 | 컴파일, 테스트 및 정적 분석 |
...
- 공통 문체와 내용은 `CORE-COMMENT-001`을 적용합니다.
- Java 전용 Javadoc, 필드 설명 및 로직 주석의 추가 기준은 7장과 8장을 함께 적용합니다.
...
- 메서드명은 영문 식별자 문자 수를 기준으로 25자 이하로 작성합니다.
- 길이를 줄일 때는 클래스와 도메인 문맥에서 이미 드러나는 반복어를 제거하고 `Cnt`, `Dtl`, `Avg`, `Sec`처럼 의미가 분명한 프로젝트 표준 축약어를 사용합니다.
- 의미를 훼손하는 임의 절단, 일련번호 또는 해시 접미사로 길이 제한을 맞추지 않습니다.
- 외부 프레임워크나 인터페이스 계약으로 이름이 고정된 오버라이드 메서드만 예외로 하며 `@Override`로 계약 근거를 명시합니다.
