...
| 규칙 ID | 수준 | 적용 절 | 주요 검증 |
| --- | --- | --- | --- |
| `VIEW-COMMENT-001` | MUST | 1~5 | 화면 영역과 실제 문구 주석 검토 |
| `VIEW-AUDIT-001` | MUST | 6 | 관리자 상세 감사정보 배치 검토 |
| `VIEW-SEARCH-001` | MUST | 7 | 목록 검색 구조와 조건 복원 검토 |

## 1. 공통 주석 작성 규칙

- 일반 주석과 JSDoc의 공통 문체와 내용은 `CORE-COMMENT-001`을 적용합니다.
- 화면 영역과 사용자 노출 문구의 추가 주석 형식은 2장부터 5장까지 함께 적용합니다.
...
- 관리자 단건 상세 화면의 등록자, 수정자, 배포자와 각 처리 일시는 본문 편집 영역 아래의 감사정보 표에 배치합니다.
- 관리자 단건 상세 화면의 테두리가 있는 `detail-panel`에는 기본정보와 본문 및 해당 내용을 선택하는 버전 이력 등 핵심 조회·편집 영역을 배치합니다.
- 공통 `AuditInfoTable`과 목록·삭제·저장·수정·배포 등 하단 명령 버튼 영역은 `detail-panel` 테두리 밖에 배치합니다.
- 버전 이력처럼 현재 상세 내용을 선택하거나 전환하는 표는 `detail-panel` 내부의 기본정보와 본문 다음에 배치합니다.
- 등록, 수정 또는 배포 개념이 없는 화면은 해당 행을 생략할 수 있으며, 기능이 존재하는데 값을 아직 기록하지 않은 경우에는 행을 유지하고 빈 값으로 표시합니다.
- 감사정보 표는 화면 하단의 목록 및 저장 등 명령 버튼 영역 바로 위에 배치합니다.
- 같은 감사정보를 화면 상단, 본문 입력 표 또는 목록 그리드에 중복 배치하지 않습니다.
- 공통 `AuditInfoTable`을 우선 재사용하고 업무별 추가 이력은 선택 속성으로 확장합니다.

## 7. 관리자 목록 검색 영역 일관성 규칙

- 관리자 목록의 검색 영역은 공통 `list-search` 구조와 `list-search-actions` 버튼 영역을 사용하고 화면별로 별도 검색 폼 스타일을 만들지 않습니다.
- 서비스 정보 관리의 검색 영역은 `PopupContentListPage`의 팝업 관리 검색 영역을 기준으로 라벨, 입력 필드, 셀렉트, 초기화 및 검색 버튼의 구조와 간격을 동일하게 구성합니다.
- 새 관리자 목록 화면을 만들 때는 업무 조건이 가장 유사한 기존 관리 화면의 검색 구조와 공통 클래스를 먼저 재사용합니다.
- 관리자 목록 검색용 셀렉트는 최초 진입과 초기화 시 빈 검색값에 대응하는 `전체` 옵션을 기본값으로 사용합니다.
- 상세 화면에서 목록으로 돌아올 때는 기본값으로 덮어쓰지 않고 사용자가 마지막으로 적용한 검색조건을 복원합니다.
